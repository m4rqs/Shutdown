...
typedef struct _THREADDATA_PACKET {
    HANDLE hThread;
    DWORD dwThreadId;
    DWORD dwThreadNr;
    BOOL fActive;
    HANDLE hPipe;
} THREADDATA_PACKET, *LPTHREADDATA_PACKET;
#define THREADDATA_PACKET_SIZE			sizeof(THREADDATA_PACKET)

//SHDWNDATA_PACKET ashDataRead[MAX_THREADS], ashDataWrite[MAX_THREADS];
MSGDATA_PACKET aMsgData[MAX_THREADS];
THREADDATA_PACKET aThreadData[MAX_THREADS];
HANDLE hMsgSendEvent[MAX_THREADS];
UINT uiActiveThreads;
