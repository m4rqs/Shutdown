...
{  
    ...
    //{
        ...
        // Musi byæ w pêtli na wypadek restartu serwisu
        do {
            fSuccess = CallNamedPipe( 
                LPCTSTR_SHDWNPIPE_NAME,         // pipe name 
                &sMsgData,                      // message to server 
                MSGDATA_PACKET_SIZE,	        // message length 
                &sMsgData,	                    // buffer to receive reply 
	        ...
	        { 
		        ...
		        if (!fSuccess) 
		        {
			        //SaveLogInfo("B³¹d w komunikacji z menad¿erem (ID:%i). Kod b³êdu: %i", dwThreadNumer, dwErrorCode);
			        //ashDataRead[dwThreadNumer].dwMessageCode = SHDWNCODE_ERROR;
		        }
		        else
		        {
			        //SaveLogInfo("Kod odpowiedzi od menad¿era (ID:%i): %i%s", dwThreadNumer, ashDataRead[dwThreadNumer].dwMessageCode, (ashDataRead[dwThreadNumer].dwMessageCode == 1) ? " (Roger That)" : "");
		        }
                break;
	        }
	        else
	        {
		        //SaveLogInfo("Menad¿er (ID:%i) nie odpowiada. Kod b³êdu: %i", dwThreadNumer, dwErrorCode);
		        ...
		        //ashDataRead[dwThreadNumer].dwMessageCode = SHDWNCODE_CANCEL;
                break;
	        }
        }
        // Na wypadek gdyby ktoœ zamkn¹³ menad¿era wyœwietlaj¹cego komunikat o zamykaniu
